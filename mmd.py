#!/usr/bin/env python
# encoding: utf-8
# source: https://github.com/OctoberChang/MMD-GAN/blob/master/mmd_gan.py
...
def mix_rbf_mmd2_fixed(X, Y):
    K_XX, K_XY, K_YY, d = _mix_rbf_kernel(X, Y, [1., 2., 4., 8., 16.])
    # return _mmd2(K_XX, K_XY, K_YY, const_diagonal=d, biased=biased)
    return _mmd2(K_XX, K_XY, K_YY, const_diagonal=False, biased=False)
...
def _mmd2(K_XX, K_XY, K_YY, const_diagonal=False, biased=False):
    m = K_XX.size(0)    # shape X
    n = K_YY.size(0)    # shape Y
...
if __name__ == "__main__":
    x = torch.randn(400, 3, requires_grad=True).cuda()
    y = torch.randn(6, 3).cuda()
    mmd2 = mix_rbf_mmd2(x, y, [1., 2., 5.])
    print(mmd2)
